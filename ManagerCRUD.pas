...
type
  ...
  private
    ...
    FCustomSQL: String;
  public
    constructor Create(AQuery: TFDQuery; ATableName: String; ACustomSQL: String = '');
...
constructor TManagerCRUD.Create(AQuery: TFDQuery; ATableName: String; ACustomSQL: String);
begin
  FQuery := AQuery;
...
begin
  ...
  FQuery.SQL.Text := Format('INSERT INTO %s (%s) VALUES (%s)', [FTableName, FieldsList, ParamsList]);
...
begin
  ...
  FQuery.SQL.Text := Format('UPDATE %s SET %s WHERE id = :id', [FTableName, SetClause]);
  FQuery.ParamByName('id').AsInteger := AID;
...
begin
  FQuery.SQL.Text := Format('SELECT * FROM %s WHERE %s LIKE :keyword', [FTableName, AField]);
  FQuery.ParamByName('keyword').AsString := '%' + AKeyword + '%';
  FQuery.Open;
end;
...
begin
  // ≈сли передан кастомный SQL-запрос, используем его
  if FCustomSQL <> '' then
  begin
